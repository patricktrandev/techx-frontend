...
export class DetailsproductAdminComponent {
  ...
  constructor(private productService:ProductService, private toaster: ToastrService, private router: Router, private route: ActivatedRoute){
  }
  changeName(){}
  changePrice(){}
  changeDescription(){}
  ...
  updateProduct(id:number){
    ...
    const val1 = (productUpdated.name! === this.productDetails?.name!)&& (productUpdated.description! === this.productDetails?.description!) 
     const val3=productUpdated.price === this.productDetails?.price! 
     const val4= productUpdated.category_id ===this.productDetails?.category.id!
   
    if( (val1&& val3 &&val4)){
      this.toaster.error("Invalid data updated", "Error", {closeButton:true, positionClass:'toast-top-center'})
      return ;

    }
    console.log(productUpdated)
    this.productService.updateProductDetailsByAdmin(id, productUpdated).subscribe({
      next:(response:ProductDetailsResponse)=>{
        //debugger
        console.log(response)
        this.toaster.success("Updated Product Successfully", "Success", {closeButton:true, positionClass:'toast-top-center'})
